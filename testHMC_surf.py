...
# Make surface
surface = getsurf(200,200,10)
writesurf('surface.pdb',surface)
nsurf = len(surface)
nspint = nsurf*numbeads

# Recenter protein
com = sum(coord, axis = 0)/numbeads
coord -= com
coord[:,2] += energyfunc.radgyr(coord)*1.7
...
r2surf = getr2surf(coord,surface,numbeads,nspint)
surfE = surfenergy(r2surf, surfparam)
...
force=cangleforces(coord,angleparam,bonds,d,numbeads)+cdihedforces(torsparam,bonds,d2,d,numbeads)+cnonbondedforces(mpos,numint,numbeads,nativeparam_n,nonnativeparam,nnepsil)+getsurfforce(coord,surface,nspint,numbeads,surfparam)
a=transpose(force)/m
...
plt.plot(range(nsteps+1),searray,label='surface-protein energy')
plt.plot(range(nsteps+1),K+pot+searray,label='Total Energy')
plt.legend()
