...
while 1:
    ...
    if splitline[0]=='HETATM':
        ...
        i=i+1
    k=k+1
#print(ptemp) #temporary

u0=energy(coord)
writepdb(coord,wtemp,hetatm,1,filename)
energyarray[0]=u0
randcoildist[0]=enddist(coord)
 ...
 #       if u1< u0:
  #          break
   ...
   #     if random()<boltz:
    ...
    randcoildist[move-1]=enddist(coord)

f=open(filename,'a')
f.write('END\r\n')
f.close

savetxt('output.txt',randcoildist)
...
#plt.show()
plt.hist(randcoildist,100,normed=1)
plt.title(str(totmoves)+' moves random coil')
